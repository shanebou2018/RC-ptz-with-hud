...
**PiP toggle (swap which camera is the inset)**: the old plan (GStreamer `compositor` pad property push, changed live with no interruption) no longer applies since GStreamer isn't in this path. The current `ffmpeg`-based pipeline instead keeps **both `rpicam-vid` processes running for `pip_stream.sh`'s entire lifetime** — they're never restarted for a swap, since Raspberry Pi camera hardware init (sensor mode negotiation, AWB/AGC convergence) takes 1-3+ seconds and would dominate swap latency if repeated on every swap. Only the much lighter `ffmpeg` composite/encode stage restarts, with `MAIN_CAM`/`INSET_CAM`'s roles swapped by changing which FIFO feeds which `ffmpeg` input — confirmed via a scripted test to restart in ~4ms (kill-to-respawn), with the cameras' own processes untouched across repeated swaps. While `ffmpeg` is down between restarts, the still-running `rpicam-vid`s just block on their `write()` calls (standard named-pipe backpressure) rather than erroring, and resume instantly once the new `ffmpeg` reopens the pipes.

Implemented as: `pip_stream.sh` runs a loop and writes its own PID to `/tmp/rc-hud-pip-stream.pid` on startup; sending it `SIGUSR1` (e.g. `kill -USR1 $(cat /tmp/rc-hud-pip-stream.pid)`) makes it swap and restart just the `ffmpeg` stage, without systemd ever seeing more than one long-running process (no service restart, no sudo needed). The web app exposes this as `POST /api/pip/swap` (`web/app.py`, reads the PID file and signals it — 503 if `pip_stream.sh` isn't running), and the HUD page's **P key** calls that endpoint, then re-negotiates its own WHEP/WebRTC connection ~800ms later (`swapPip()` in `index.html`) — MediaMTX treats the new `ffmpeg` publish as a fresh stream that an already-connected browser won't pick up on its own, so without this the page would need a manual refresh after every swap (confirmed by hitting exactly that before this was added). `startVideo()` now always resolves (catches its own network/WHEP errors into the status badge) specifically so this re-negotiation call can't leave the badge stuck on "swapping cameras…" if MediaMTX is genuinely unreachable.

Verified with a scripted test (stub `rpicam-vid`/`ffmpeg` commands standing in for the real binaries, which don't exist on this dev machine): `SIGUSR1` correctly swaps camera roles and swaps back **without ever restarting the two `rpicam-vid` processes** (confirmed by their PIDs and start-timestamps staying identical across 3 repeated swaps, only `ffmpeg`'s PID changing each time), `SIGTERM` cleanly kills all children with no orphaned processes or leftover FIFOs, and the `/api/pip/swap` endpoint correctly signals a real process (503 for a dead one). Also verified end-to-end in a browser (Playwright): pressing **P** triggers a second WHEP negotiation request ~800ms later as intended, and a simulated MediaMTX-unreachable case correctly surfaces as a status-badge error instead of hanging on "swapping cameras…" forever. **Confirmed working on real hardware**: pressing P swaps the camera roles with no manual page refresh needed.
...
- **P** — swap which camera is the PiP inset, momentary (ignores key-repeat). See "PiP toggle" above for how this actually reaches `pip_stream.sh`.
...
- `web/` — FastAPI app serving the HUD page (`web/static/index.html`): WHEP video embed, canvas HUD (compass dial, servo readout, motor bars), and on-screen servo/motor controls, all over the control websocket. Also exposes `POST /api/pip/swap` (see "PiP toggle" above) — the one HTTP endpoint on this app that isn't just serving the static page.
