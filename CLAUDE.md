...
**PiP toggle (swap which camera is the inset)**: the old plan (GStreamer `compositor` pad property push, changed live with no interruption) no longer applies since GStreamer isn't in this path — the current `ffmpeg`-based pipeline has to actually restart its capture to swap roles. Implemented as: `pip_stream.sh` runs its capture/composite/encode in a loop and writes its own PID to `/tmp/rc-hud-pip-stream.pid` on startup; sending it `SIGUSR1` (e.g. `kill -USR1 $(cat /tmp/rc-hud-pip-stream.pid)`) makes it tear down and restart its two `rpicam-vid` processes + `ffmpeg` with `MAIN_CAM`/`INSET_CAM`'s roles flipped, without systemd ever seeing more than one long-running process (no service restart, no sudo needed). The web app exposes this as `POST /api/pip/swap` (`web/app.py`, reads the PID file and signals it — 503 if `pip_stream.sh` isn't running), and the HUD page's **P key** calls that endpoint. Expect a few seconds of video interruption during the swap, same as any camera restart — the status badge shows "swapping cameras…" and reverts after a guessed delay (there's no completion signal from the Pi, see `swapPip()` in `index.html`).

Verified with a scripted test (stub `rpicam-vid`/`ffmpeg` commands standing in for the real binaries, which don't exist on this dev machine): `SIGUSR1` correctly swaps camera roles and swaps back, `SIGTERM` cleanly kills all children with no orphaned processes or leftover FIFOs, and the `/api/pip/swap` endpoint correctly signals a real process (and returns 503 for a dead one). **The signal-handling logic is verified; the actual on-hardware camera swap (pressing P and confirming the video really switches) is not yet tested.**
