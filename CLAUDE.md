...
- Camera #2 is now physically attached, and `pipeline/pip_stream.sh` is **confirmed working on real hardware** — composited PiP video (camera 0 full-frame, camera 1 as a 480×270 inset) confirmed live in VLC. `systemd/rc-hud-pipeline.service` still points at `single_cam_stream.sh`; switch it to `pip_stream.sh` (with `MAIN_CAM`/`INSET_CAM` env vars set) whenever ready to go dual-camera by default.
